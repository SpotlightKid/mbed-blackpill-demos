...
void Joystick::init(float x_offset, float y_offset) {
    // read centered values of joystick

    // this assumes that the joystick is centered when the init function is
    // called if perfectly centered, the pots should read 0.5, but this may not
    // be the case and x0 and y0 will be used to calibrate readings
    _x0 = horiz->read() + x_offset;
    _y0 = vert->read() + y_offset;
}

Direction Joystick::get_direction() {
    float angle = get_angle();  // 0 to 360, -1 for centered

    Direction d;
    // partition 360 into segments and check which segment the angle is in
    if (angle < 0.0f) {
        d = CENTER;              // check for -1.0 angle
...
Vector2D Joystick::get_coord() {
    // read() returns value in range 0.0 to 1.0 so is scaled and center value
...
Polar Joystick::get_polar() {
    ...
    // around the centered values. This causes the random angle values to get
    // calculated when the joystick is centered and untouched. This is also when
    // the magnitude is very small, so we can check for a small magnitude and
    // then set the angle to -1. This will inform us when the angle is invalid
    // and the joystick is centered
